...
#include "ESP8266githubUpdate.h"
#include <StreamString.h>

extern "C" uint32_t _FS_start;
extern "C" uint32_t _FS_end;

ESP8266githubUpdate::ESP8266githubUpdate(void)
    : _httpClientTimeout(8000)
{
}

ESP8266githubUpdate::ESP8266githubUpdate(int httpClientTimeout)
 ...
 */
void ESP8266githubUpdate::setAuthorization(const String &user, const String &password)
{
    _user = user;
    _password = password;
}
 ...
 */
void ESP8266githubUpdate::setAuthorization(const String &auth)
{
    _auth = auth;
}

HTTPUpdateResult ESP8266githubUpdate::update(WiFiClient &client, const String &url, const String &currentVersion)
{
    HTTPClient http;
...
HTTPUpdateResult ESP8266githubUpdate::updateFS(WiFiClient &client, const String &url, const String &currentVersion)
{
    HTTPClient http;
    http.begin(client, url);
...
HTTPUpdateResult ESP8266githubUpdate::update(WiFiClient &client, const String &host, uint16_t port, const String &uri,
 ...
 */
int ESP8266githubUpdate::getLastError(void)
{
    return _lastError;
}
 ...
 * @return String error
 */
String ESP8266githubUpdate::getLastErrorString(void)
{
 ...
 */
HTTPUpdateResult ESP8266githubUpdate::handleUpdate(HTTPClient &http, const String &currentVersion, bool spiffs)
{
 ...
 */
bool ESP8266githubUpdate::runUpdate(Stream &in, uint32_t size, const String &md5, int command)
{
...
// For github update
ESP8266githubUpdate::ESP8266githubUpdate(String _repo, String _token, String _cert)
{
    repo = _repo;
    token = _token;
    certificate = _cert;
...
String ESP8266githubUpdate::handleVersion(HTTPClient &http)
{
    http.useHTTP10(true);
...
String ESP8266githubUpdate::getVersion(bool _private, String _filename)
{
    WiFiClientSecure client;
    HTTPClient http;
    X509List cacert(certificate.c_str());

    gitHub_private = _private;
    ...
    if (gitHub_private)
    {
        url = "https://api.github.com/repos/" + repo + "/contents/" + _filename;
    }
    else
    {
        url = "https://raw.githubusercontent.com/" + repo + "/master/" + _filename;
    }
    Serial.println(url);
    http.begin(client, url);
...
t_httpUpdate_return ESP8266githubUpdate::git_update(bool _private, const String &file)
{
    WiFiClientSecure client;
    HTTPClient http;
    X509List cacert(certificate.c_str());
...
ESP8266githubUpdate ESPgithubUpdate;
#endif
